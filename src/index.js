...
class WaterHeaterCard extends LitElement {
  static get styles() {
    return styles
  }
  ...
  set hass(hass) {
    ...
    this.modes = controlModes.map(values => {
      if (values.type === 'water_heater') {
        const sortedList = []
        values.list.forEach(item => {
          const index = WATER_HEATER__MODES.indexOf(item.value)
  ...
  renderModeType({ type, mode = 'none', list, name }) {
    ...
    let localizePrefix = `state_attributes.water_heater.${type}_mode.`
    if (type === 'hvac') {
      localizePrefix = `state.water_heater.`
    }
    ...
    const title = name || this.localize(`ui.card.water_heater.${str}`)
  ...
  // Operation mode can be one of: none, eco, electric, performance, high_demand, heat_pump, gas
  // See https://github.com/home-assistant/core/blob/dev/homeassistant/components/water_heater/__init__.py
  ...
  setMode(type, mode) {
    if (type && mode) {
      this._hass.callService('water_heater', `set_${type}_mode`, {
        entity_id: this.config.entity,
        [`${type}_mode`]: mode,
      })
...
window.customElements.define('water-heater-card', WaterHeaterCard)
